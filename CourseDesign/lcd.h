...
#define LCD1602_4PINS // 设置为4位数据接口
...
extern void lcdWriteComd(byte cmd);
extern void lcdWriteData(byte dat);
extern void lcdWaitBusy(void);
extern void lcdInit(void);
extern void lcdPutchar(char c);
extern void lcdPuts(const char *str);
extern void lcdPutDigit(uint16 num);

/*!
    move cursor to pos
*/
#define lcdMoveCursor(pos)           \
    do                               \
    {                                \
        lcdWriteComd(CMD_CURES);     \
        lcdWriteComd(DDRAMADD(pos)); \
    } while (0)
